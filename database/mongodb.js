 ...
 * @param callback 回调方法
 */
DB.prototype.saveDocs = function (table_name, docs, callback) {
    if (!docs) {
        if (callback) callback({msg: 'docs is not allowed for null'});
        return false;
    }

    var node_model = this.getConnection(table_name);
    
    node_model.collection.insert(docs, function (err, res) {
        if (err) {
            if (callback) callback(err);
        } else {
            if (callback) callback(null, res);
 ...
 * @param callback 回调方法
 */
DB.prototype.find = function (table_name, conditions, fields, sort, callback) {
    var node_model = this.getConnection(table_name);
    if(sort !== null){
        node_model.find(conditions, fields || null, {}).sort(sort).exec(function (err, res) {
            if (err) {
                callback(err);
            } else {
                callback(null, res);
        ...
        node_model.find(conditions, fields || null, {}).exec(function (err, res) {
            if (err) {
                callback(err);
            } else {
                callback(null, res);
 ...
 * @param table_name 表名
 * @param conditions 查询条件 {a:1, b:2}
 * @param options 选项：{sort: {time: -1}, pageSize: 10, page: 2}
 * @param callback 回调方法
 */
DB.prototype.findPage = function (table_name, conditions, options, callback) {
    var node_model = this.getConnection(table_name);
    node_model.find(conditions)
        .sort(options.sort || {})
